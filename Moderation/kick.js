...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (!message.guild.members.cache.get(Member.id))
      return message.lineReplyNoMention(`Please Mention A Valid Member!`);

    if (Member.id === message.author.id)
      return message.lineReplyNoMention(`You Can't Kick Your Self!`);

    if (Member.id === client.user.id)
      return message.lineReplyNoMention(`Please Don't Kick Me ;-;`);

    if (Member.id === message.guild.owner.user.id)
      return message.lineReplyNoMention(`You Can't Kick Owner Of Server!`);
    ...
    if (!User.kickable)
      return message.lineReplyNoMention(`I Can't Kick That Member!`);
